...
const packageFileStem = packageMetadata.name.replace(/^@/, "").replaceAll("/", "-");
const packageTarballName = `${packageFileStem}-${packageMetadata.version}.tgz`;
const packageRoot = packageLock.packages?.[""];
assert.equal(packageMetadata.name, "@cesaremcasa/tokenpilot");
...
function runPack(destination) {
  ...
  assert.equal(record?.filename, packageTarballName);
...
try {
  ...
  const sbom = path.join(outputDirectory, `${packageFileStem}-${packageMetadata.version}.cdx.json`);
