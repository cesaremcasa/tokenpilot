...
The report separates measured coverage from unavailable sessions before displaying reduction. A result is validated only for classified non-benchmark work with at least five measured baseline and five measured treatment sessions in the same provider, task type, measurement basis, policy version, and price-profile snapshot. `unknown` and benchmark rows remain preliminary. There is no pre-declared savings percentage. Any later decision is based on measured results and quality signals, not a token-reduction promise.
