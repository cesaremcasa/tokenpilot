...
function assertSafeDirectory(target: string): void {
  ...
  if (stat.isSymbolicLink() || !stat.isDirectory() || stat.uid !== currentUid() || (stat.mode & 0o022) !== 0 || hasUnsafeMacAcl(target)) {
...
export function assertSafeStateFile(paths: TokenPilotPaths, file: string): void {
  ...
  try {
    ...
    if (stat.isSymbolicLink() || !stat.isFile() || stat.nlink !== 1 || stat.uid !== currentUid() || hasUnsafeMacAcl(file)) {
